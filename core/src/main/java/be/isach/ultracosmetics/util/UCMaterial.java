...
/**
 * <b>UCMaterial v2.3.1</b> - Data Values/Pre-flattening<br>
...
public enum UCMaterial {
    ...
    UCMaterial(int data, String... legacy) {
        this.data = (byte) data;
        this.legacy = legacy;
    }
     ...
     *
     * @see #matchUCMaterial(String, byte)
     */
    @Nonnull
    private static UCMaterial requestOldUCMaterial(@Nonnull String name, byte data) {
        UCMaterial cached = CACHED_SEARCH.get(name + "," + data);

        if (cached != null) return cached;
        Optional<UCMaterial> search = data == -1 ? Arrays.stream(UCMaterial.VALUES).filter(mat -> mat.matchAnyLegacy(name)).findFirst()
                : Arrays.stream(UCMaterial.VALUES).filter(mat -> mat.matchAnyLegacy(name) && mat.data == data).findFirst();

        if (search.isPresent()) {
            UCMaterial found = search.get();
    ...
    /**
     * Checks if UCMaterial enum contains a material with this name.
     *
     * @param name name of the material
     * @return true if UCMaterial enum has this material.
     */
    public static boolean contains(@Nonnull String name) {
        Validate.notEmpty(name, "Material name cannot be null or empty");
        String formatted = format(name);
        return Arrays.stream(UCMaterial.VALUES).anyMatch(mat -> mat.name().equals(formatted));
    }
     ...
     */
    @Nullable
    public static UCMaterial matchUCMaterial(@Nonnull Material material) {
        return matchUCMaterial(material.name());
    }
     ...
     */
    @Nullable
    public static UCMaterial matchUCMaterial(@Nonnull String name) {
        ...
        return name.contains(":") ? matchUCMaterial(name.substring(0, name.indexOf(":")),
                Byte.parseByte(StringUtils.replace(name.substring(name.indexOf(":") + 1), " ", "")))
                : matchUCMaterial(name, (byte) -1);
    }
    ...
    public static Material parseMaterial(@Nonnull String name, byte data) {
        return matchUCMaterial(name, data).parseMaterial();
    }
    ...
    public static UCMaterial matchUCMaterial(@Nonnull ItemStack item) {
        Objects.requireNonNull(item, "ItemStack cannot be null");
        return isDamageable(item.getType().name()) ? matchUCMaterial(item.getType().name(), (byte) 0) :
                matchUCMaterial(item.getType().name(), (byte) item.getDurability());
    }

    /**
     * Matches the argument string and its data with a UCMaterial.<br>
     *
     * @param name the name of the material
     * @param data the data byte of the material
     * @return a UCMaterial from the enum (with the same legacy name and data if in older versions.)
     */
    @Nonnull
    public static UCMaterial matchUCMaterial(@Nonnull String name, byte data) {
        Validate.notEmpty(name, "Material name cannot be null or empty");
        name = format(name);
        ...
        if (isDuplicated(name) && !isNewVersion())
            return requestDuplicatedUCMaterial(name, data);

        return requestOldUCMaterial(name, data);
    }
     ...
     *
     * @param name the name of the material.
     * @return the duplicated UCMaterial based on the version.
     */
    @Nullable
    private static UCMaterial requestDuplicatedUCMaterial(@Nonnull String name, byte data) {
        Validate.notEmpty(name, "Material name cannot be null or empty");
        UCMaterial mat = requestOldUCMaterial(name, data);
     ...
     *
     * @param name the name of the material.
     * @return the new UCMaterial of this duplicated material.
     * @see #getUCMaterialIfDuplicated(String)
     */
    @Nullable
    public static UCMaterial getNewUCMaterialIfDuplicated(@Nonnull String name) {
        Validate.notEmpty(name, "Material name cannot be null or empty");
     ...
     *
     * @param name the name of the material.
     * @return the other UCMaterial (key or value) of the UCMaterial (key or value) which the name was equals to.
     * @see #matchUCMaterial(String, byte)
     */
    @Nullable
    public static UCMaterial getUCMaterialIfDuplicated(@Nonnull String name) {
        Validate.notEmpty(name, "Material name cannot be null or empty");
        String formatted = format(name);
        Optional<Map.Entry<UCMaterial, UCMaterial>> mat = DUPLICATED.entrySet().stream().filter(m
                -> m.getKey().name().equals(formatted) || m.getValue().name().equals(formatted)).findFirst();

        if (mat.isPresent()) {
            Map.Entry<UCMaterial, UCMaterial> found = mat.get();
     ...
     */
    @Nullable
    public UCMaterial getUCMaterialIfDuplicated() {
        Optional<Map.Entry<UCMaterial, UCMaterial>> mat = DUPLICATED.entrySet().stream().filter(m -> m.getKey() == this).findFirst();
    ...
    /**
     * Parses the UCMaterial as an {@link ItemStack}.
     *
    ...
    /**
     * Parses the UCMaterial as a {@link Material}.
     *
     * @return the Material related to this UCMaterial based on the server version.
     */
    @Nullable
     ...
     */
    @Nullable
    public UCMaterial suggestOldMaterialIfNew() {
        ...
        for (int i = this.legacy.length - 1; i >= 0; i--) {
            ...
            UCMaterial mat = matchUCMaterial(parseLegacyVersionMaterialName(legacyName), this.data);
