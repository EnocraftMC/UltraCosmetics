    ...
    void onRightClick() {
        ...
        final int i = Bukkit.getScheduler().runTaskTimerAsynchronously(getUltraCosmetics(), () -> {
            ...
            for (int i1 = 0; i1 < 5; i1++) {
                UtilParticles.drawParticlesWithOffset(Particles.EXPLOSION_NORMAL, 0.2d, 0.2d, 0.2d, loc1, 1);
                UtilParticles.drawParticlesWithOffset(Particles.DRIP_WATER, 0.4d, 0.4d, 0.4d, loc2, 2);
            }
            for (int a = 0; a < 100; a++)
                UtilParticles.drawColoredDust(0, 0, 255, loc.clone().add(MathUtils.randomDouble(-1.5, 1.5), MathUtils.randomDouble(1, 1.6) - 0.75, MathUtils.randomDouble(-1.5, 1.5)));
            if (affectPlayers) {
